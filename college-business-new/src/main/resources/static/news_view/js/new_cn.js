...
function metpagerajax(){
	var $metpagerbtn=$("#met-pager-btn"),
		...
		metpagerbtnText=function(){
			if(pagemax){
				if(pagemax <= page && page>1) $metpagerbtn.addClass('disabled').text('å·²ç»æ˜¯æœ€åŽä¸€é¡µäº†');
			}else{
...
function metpagerajaxFun(page){
	...
	if($('#met-grid').length){
		setTimeout(function(){
			if(typeof metAnimOnScroll != 'undefined') metAnimOnScroll('met-grid');// äº§å“æ¨¡å—ç€‘å¸ƒæµ
		},100)
	}
}
/*
 ...
 */
$(function(){
	// æ–‡ç« åˆ—è¡¨é¡µæ–°é—»å¤´æ¡ï¼ˆéœ€è°ƒç”¨slickæ’ä»¶ï¼‰
